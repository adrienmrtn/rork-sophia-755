...
nonisolated struct UserProgress: Codable, Sendable {
    ...
    /// Course ID that a free user "claimed" as their one free course for `dailyFreeCourseDate`.
    /// That single course stays fully readable all day (and is revisitable); every other course
    /// opened the same day is intro-only + locked. Resets when `dailyFreeCourseDate` != today.
    var dailyFreeCourseId: String?
    /// yyyy-MM-dd date string for the day `dailyFreeCourseId` was claimed.
    ...
    init(from decoder: Decoder) throws {
        ...
        self.dailyFreeCourseId = try c.decodeIfPresent(String.self, forKey: .dailyFreeCourseId)
        self.dailyFreeCourseDate = try c.decodeIfPresent(String.self, forKey: .dailyFreeCourseDate)
