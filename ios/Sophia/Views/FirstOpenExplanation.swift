...
struct GlossaryCoachmark: View {
    ...
    private var card: some View {
        VStack(spacing: 18) {
            // Le mot exact, rendu comme dans le cours (souligné accent).
            Text(term)
                .font(DS.title(.title2, .semibold))
                .foregroundStyle(DS.ink)
                ...
                .padding(.vertical, 10)
                .background(DS.accentTint.opacity(0.5), in: RoundedRectangle(cornerRadius: DS.Radius.small, style: .continuous))
