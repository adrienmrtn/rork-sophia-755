#!/usr/bin/env python3
"""Generate Portuguese locale JSON bundles from CSV exports + French CourseData IDs."""
...
DEFAULT_OUT = ROOT / "ios/Sophia/Locales/pt"
...
SUBJECT_PT_MAP = {
    ...
    "Compreender o mundo atual": "comprendreLeMonde",
}

CLASSIFICATION_PT_MAP = {
    "Referência histórica": "referenceHistorique",
    "Conceito": "concept",
    "Evento relacionado": "evenementConnexe",
    "Evento": "evenementConnexe",
    "Personagem": "personnage",
    "Lugar": "lieuInstitution",
    "Lugar / instituição": "lieuInstitution",
    "Instituição": "lieuInstitution",
    "Movimento": "concept",
    "Obra": "referenceHistorique",
    "Outro": "concept",
    "Data": "referenceHistorique",
    "Lei": "referenceHistorique",
    "Teoria": "concept",
    "Técnica": "concept",
...
def resolve_course_titles_pt(titles: str, title_map: dict[str, str]) -> list[str] | None:
...
def build_courses_json(
    ...
    pt_rows: list[dict[str, str]],
    pt_quizzes: dict[str, list[dict[str, object]]],
) -> list[dict[str, object]]:
    courses: list[dict[str, object]] = []
    for app_course, pt_row in zip(app_courses, pt_rows):
        pt_title = (pt_row.get("Titre") or "").strip()
        subject_label = (pt_row.get("Matière") or "").strip()
        subject_key = SUBJECT_PT_MAP.get(subject_label)
        if not subject_key:
            raise ValueError(f"Unknown PT subject: {subject_label!r} for {pt_title}")

        parts: list[tuple[str, str]] = []
        for idx in range(1, 5):
            part_title = normalize_content(pt_row.get(f"Partie {idx} Titre"))
            part_content = normalize_content(pt_row.get(f"Partie {idx} Contenu"))
            if not part_content:
                continue
            parts.append((part_title or f"Parte {idx}", part_content))

        intro = normalize_content(pt_row.get("Intro"))
        quiz_questions = pt_quizzes.get(pt_title)
        ...
        lessons = [
            {"id": lesson_ids[0], "title": "Introdução", "content": intro},
        ]
        ...
        courses.append(
            {
                ...
                "subject": subject_key,
                "subcategory": normalize_content(pt_row.get("Sous-catégorie")),
                "lessons": lessons,
                "quiz": quiz,
            }
        )
...
def build_glossary_json(
    ...
    for app_course, pt_row in zip(app_courses, pt_rows):
        pt_title = (pt_row.get("Titre") or "").strip()
        course_id = str(app_course["id"])
        for row in glossary_rows.get(pt_title, []):
            classification = CLASSIFICATION_PT_MAP.get(row["classification"])
            if not classification:
                raise ValueError(
                    f"Unknown PT classification: {row['classification']!r} ({pt_title})"
                )
            term = row["term"]
...
def build_cards_json_pt(path: Path, title_map: dict[str, str]) -> list[dict[str, object]]:
    cards: list[dict[str, object]] = []
    for row in read_csv(path):
        card_id = (row.get("id") or "").strip()
        name = (row.get("Nom") or "").strip()
        rarity_label = (row.get("Rareté") or row.get("Rarete") or "").strip()
        rarity = RARITY_PT_MAP.get(rarity_label)
        course_titles = row.get("Cours") or ""
        course_ids = resolve_course_titles_pt(course_titles, title_map)
...
def build_collections_json_pt(path: Path, title_map: dict[str, str]) -> list[dict[str, object]]:
...
def parse_args() -> argparse.Namespace:
    source = ROOT / "content/locales/pt/source"
    parser = argparse.ArgumentParser()
    parser.add_argument("--courses-csv", type=Path, default=source / "Excel_cours_systeme_modifie_PT_4407.csv")
    parser.add_argument("--quiz-csv", type=Path, default=source / "Excel_QCM_PT_e5ba.csv")
    parser.add_argument("--glossary-csv", type=Path, default=source / "Glossaire_culture_generale_modifie_PT_ee5e.csv")
    parser.add_argument("--collections-csv", type=Path, default=source / "Collections_PT_60aa.csv")
    parser.add_argument("--cards-csv", type=Path, default=source / "Cartes_PT_d400.csv")
...
def main() -> None:
    ...
    pt_rows = load_en_courses_rows(args.courses_csv)
    pt_quizzes = load_en_quizzes(args.quiz_csv)
    glossary_rows = load_en_glossary(args.glossary_csv)
    pt_title_to_id, _ = build_title_maps(app_courses, pt_rows)

    courses = build_courses_json(app_courses, pt_rows, pt_quizzes)
    glossary = build_glossary_json(app_courses, pt_rows, glossary_rows)
    collections = build_collections_json_pt(args.collections_csv, pt_title_to_id)
    cards = build_cards_json_pt(args.cards_csv, pt_title_to_id)
